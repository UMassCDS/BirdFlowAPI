...
symbolize_raster_data <- function(tiff = NULL,
  ...
  if (is.null(max_value)) {
    max_value <- max(terra::values(r), na.rm = TRUE)
  }
  ...
  red <- green <- blue <- r
  terra::values(red) <- vc[, 1]
  terra::values(blue) <- vc[, 2]
  terra::values(green) <- vc[, 3]
  rgb <- c(red, blue, green)

  terra::RGB(rgb) <- 1:3

  terra::writeRaster(rgb,
    filename = png, filetype = "PNG",
