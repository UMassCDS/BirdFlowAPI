...
get_s3_config <- function() {
  config_option_list <- config$option %||% list()
  access_key <- config_option_list$access_key %||% Sys.getenv("AWS_ACCESS_KEY_ID", unset = NA)
  secret_key <- config_option_list$secret_key %||% Sys.getenv("AWS_SECRET_ACCESS_KEY", unset = NA)
  region <- config_option_list$region %||% Sys.getenv("AWS_DEFAULT_REGION", unset = NA)
  bucket <- config_option_list$bucket %||% Sys.getenv("S3_BUCKET_NAME", unset = NA)
