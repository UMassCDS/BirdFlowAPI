...
def upload_processed_data():
    ...
    for filename in os.listdir(processed_data_dir):
        if filename.endswith('.json'):
            ...
            # Add timestamp to filename for history
            base_name = filename.replace('.json', '')
            timestamped_filename = f"{base_name}_{timestamp}.json"
            history_key = f"outbreaks/history/{timestamped_filename}"
            # Key for always-updated root version
            root_key = f"outbreaks/{filename}"
            # Upload to history (versioned)
            upload_to_s3(local_path, history_key)
            # Upload to root (overwrite previous)
            upload_to_s3(local_path, root_key)
